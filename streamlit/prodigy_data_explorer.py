...
dataset = st.sidebar.selectbox(
    f"Datasets ({len(db_sets)})", [placeholder] + db_sets)
if dataset != placeholder:
    ...
    if not len(examples):
        ...
        set_type = st.sidebar.selectbox(
            "Dataset type", dataset_types, guessed_index)
        fields = list(examples[0].keys())
        default_fields = [f for f in fields if f[0]
                          != "_" and f not in EXC_FIELDS]
        task_fields = st.sidebar.multiselect(
            "Visible fields", fields, default_fields)
        ...
        if set_type in ["ner", "textcat"]:
            ...
            spacy_model = st.sidebar.selectbox(
                spacy_model_title, SPACY_MODEL_NAMES)
            ...
            eval_split = st.sidebar.slider(
                eval_split_label, 0.0, 1.0, 0.2, 0.05)
        ...
        if set_type == "ner":
            ...
            for span in ner_example.get("spans", []):
                ...
                ents.append(doc.char_span(
                    span["start"], span["end"], label=label))
            doc.ents = filter_spans(ents)
            html = displacy.render(
                doc, style="ent", options={"colors": colors})
            ...
            if st.button("Start training"):
                ...
                st.success(
                    f"对勾 Using **{len(train_examples)}** training examples "
                ...
                for i in range(n_iter):
                    ...
                    result_table.dataframe(
                        results_df.style.applymap(highlight))
